...
# Used for testing RP gradient estimator values

def rp_linear_foo(mean, var, eps):
    rp_sample = mean + jnp.sqrt(var) * eps
    return linear_foo(rp_sample)
    
def get_rp_linear_foo_gradients(means, variances, N, num_inputs, key=RAND_KEY):
    epsilons = jrandom.normal(key=key, shape=(num_inputs,N))

    mu_grad = jax.vmap(
        jax.vmap(grad(rp_linear_foo, 0), in_axes=(0, 0, 0)), in_axes=(0, 0, 0)
    )
    var_grad = jax.vmap(
        jax.vmap(grad(rp_linear_foo, 1), in_axes=(0, 0, 0)), in_axes=(0, 0, 0)
    )

    mu_grads = mu_grad(means, variances, epsilons)
    var_grads = var_grad(means, variances, epsilons)

    return mu_grads, var_grads

def get_rp_linear_foo_grad_var(means, variances, N, num_inputs, num_batches=NUM_BATCHES):
    mu_grads = np.zeros((num_batches, num_inputs))
    var_grads = np.zeros((num_batches, num_inputs))
    ...
    for i in range(num_batches):
        mu_grad, var_grad = get_rp_linear_foo_gradients(
                                    means,
                                    variances,
                                    N,
                                    num_inputs,
